...
class ClassConditionerAdd(nn.Module):
    def __init__(self, embedding: nn.Embedding, class_to_idx: dict[str, int], output_shape: tuple[int, int, int]):
        super().__init__()
        self.embedding = embedding
        self.class_to_idx = class_to_idx
        self.linear = nn.Linear(embedding.embedding_dim, output_shape[0] * output_shape[1] * output_shape[2])
...
class ClassConditionerAdd(nn.Module):
    ...
    def forward_add(self, class_labels: list[str], device: str, image: torch.Tensor) -> torch.Tensor:
        assert image.shape[1:] == self.output_shape, f"Expected image shape {self.output_shape}, but got {image.shape[1:]}"
        class_idx = torch.tensor([self.class_to_idx[c] for c in class_labels], device=device, dtype=torch.long)
        x = self.embedding(class_idx)  # [B, emb_dim]
        x = self.linear(x)  # [B, hidden_dim]
        x = x.view(-1, *self.output_shape)  # [B, *output_shape]
        return x + image

    def forward_concat(self, class_labels: list[str], device: str, image: torch.Tensor) -> torch.Tensor:
        class_idx = torch.tensor([self.class_to_idx[c] for c in class_labels], device=device, dtype=torch.long)
...
def main_vae(
        ...
        batch_size=128,
        num_epoch=20000,
        output_dir: Path = Path("outputs/vae3_reconstructions"),
        num_samples=20,
        learning_rate=1e-4,
        class_conditioning_input: Literal["deactivated", "add", "concat", "concat-retain-dim"] = "concat",
        class_conditioning_latent: Literal["deactivated", "add", "concat", "concat-retain-dim"] = "add",
        vae_latent_dim=32,
    ...
    vae_small = AutoencoderKL(
        in_channels=4,
        out_channels=4,
        latent_channels=vae_latent_dim,
        sample_size=encoded_image_size,
    )

    latent_example = vae_small.encode(torch.randn(1, 4, encoded_image_size, encoded_image_size)).latent_dist.sample()
    ...
    for epoch in tqdm(range(num_epoch)):
        ...
        for i, batch in enumerate(bar := tqdm(train_dataloader, leave=False)):
            ...
            latent_dist = vae_small.encode(encoded_image).latent_dist
            cls_emb = class_conditioner(class_labels, device)  # [B, latent_dim]
            x = latent_dist.sample() + cls_emb
            decoded = vae_small.decode(x).sample  # [B, 4, H, W]
